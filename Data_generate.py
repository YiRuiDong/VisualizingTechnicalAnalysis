...
def Save_img(tech1: str, tech2: str):
    tech_list = [tech1, tech2]
    print(tech_list)
    d = Data_generator('./data_us/tech/', market='us')
    d.register_sample_csv_list(250 + 74 + 5)
    if not op.exists('N:/Dataset/img/74window5day'):
        existed_i = 0
        existed_j = 0
    else:
        name_list = os.listdir('N:/Dataset/img/74window5day')
        code = [i[:5] for i in d.csv_name_list]
        name_list_copy = copy.copy(name_list)
        for i in name_list_copy:
            if i[:5] not in code:
                name_list.remove(i)
        if len(name_list) == 0:
            existed_i = 0
            ...
            mask = [1 for i in range(len(name_list))]
            for i in range(len(name_list)):
                tech_list = os.listdir(op.join('N:/Dataset/img/74window5day', name_list[i]))
                if tech1 + tech2 not in tech_list:
                    mask[i] = 0
            code_list = [int(name_list[i][:5]) if mask[i] == 1 else 0 for i in range(len(name_list))]
            existed_i = code_list.index(max(code_list))
            if not op.exists(
                    op.join('N:/Dataset/img/74window5day/', f'{d.csv_name_list[existed_i][:5]}/{tech1}+{tech2}')):
                existed_j = 0
            else:
                img_list = os.listdir(
                    op.join('N:/Dataset/img/74window5day/', f'{d.csv_name_list[existed_i][:5]}/{tech1}+{tech2}'))
                if len(img_list) > 0:
                    index_list = [int(filename.split('_')[0]) for filename in img_list]
                    existed_j = index_list.index(max(index_list))
                else:
                    existed_j = 0
    for i in range(existed_i, len(d.csv_name_list)):
        isGet = d.read_data(i, 'sample')
        if isGet is None:
            continue
        if (existed_i != 0) and (i == existed_i):
            print(f'start reading from {d.csv_name_list[existed_i]}')
        print(f'now converting {d.csv_name_list[i]}')
        if len(d.df) < 250 + 74 + 5:
            print('not enough length, jump this one')
            continue
        for j in range(existed_j, len(d.df) - 250 - 74 - 5):
            if (existed_j != 0) and (j == existed_j):
                print(f'index before {existed_j} already exists')
            data_zip = d.sep_OHLC_Techs_Label(249 + j, 74, 5)
            label = data_zip['label']
            DrawLib.DrawOHLCTechnical(224, data_zip['OHLC'],
                                      f'N:/Dataset/img/74window5day/{d.csv_name_list[i][:5]}/{tech1}+{tech2}',
                                      filename=f'{j}_{label}.bmp',
                                      MA=data_zip['MA'] if 'MA' in tech_list else None,
                                      MACD=data_zip['MACD'] if 'MACD' in tech_list else None,
                                      Vol=data_zip['Vol'] if 'Vol' in tech_list else None,
                                      BOLL=data_zip['BOLL'] if 'BOLL' in tech_list else None,
                                      RSI=data_zip['RSI'] if 'RSI' in tech_list else None,
                                      show=False)
        existed_j = 0
